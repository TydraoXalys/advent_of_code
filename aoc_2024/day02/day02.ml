...
let is_safe_with_dampener report =
  ...
  (*  Checks if the report is safe at the first iteration if we remove the first or the second level.
    ...
    * - report        : List of integers that represent the remaining part of the report to analyse.
    *)
  let check_first_iteration lvl1 lvl2 next_lvl report =
    ...
    (*  Gets the value of previous_lvl for the second iteration.
      ...
      * - next_lvl      : Level that follows lvl2 in the entire report.
      *)
    let get_previous_level lvl1 lvl2 next =
      if 
        is_snd_rule_broken lvl1 next 
      then 
        lvl2
      else 
        lvl1
    ...
    in if 
      is_snd_rule_broken lvl1 next_lvl && 
      is_snd_rule_broken lvl2 next_lvl
    then 
      false
    else
      let previous = get_previous_level lvl1 lvl2 next_lvl
      in next_iteration (next_lvl::report) previous (compare previous next_lvl) true
  ...
  in match report with
    ...
    | _ :: []                                                         -> true
    | _ :: _ :: []                                                    -> true
    | lvl1 :: lvl2 :: next_lvl :: q when is_snd_rule_broken lvl1 lvl2 -> check_first_iteration lvl1 lvl2 next_lvl q
    | lvl1 :: lvl2 :: q                                               -> next_iteration (lvl2::q) lvl1 (compare lvl1 lvl2) false
